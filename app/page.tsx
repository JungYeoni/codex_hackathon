...
export default function Home() {
  ...
  return <main className="app-shell">
    <nav className="topbar"><button className="brand ghost-button" onClick={() => { setActiveNav("new-analysis"); setStep(1); }}><span className="brand-mark">S</span>살펴봄</button><div className="nav-progress"><span className="pulse" />{step === 1 ? "구매 판단을 더 선명하게" : step === 2 ? "여러 후보를 함께 비교해요" : "구매 판단을 더 선명하게"}</div><span className="page-label">0{step} / 03 {step === 1 ? "구매 기준" : step === 2 ? "매물 등록" : "비교 판단"}</span></nav>
